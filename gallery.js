...
for (const gallery of galleries) {
                    ...
                    if (this.readyState == 4 && this.status == 200) {
                        ...
                        if (meta.date) {
                            if (meta.date.timeString) {
                                ...
                                img.dataset.date = `${date.getFullYear()}.${date.getMonth()}.${date.getDate()} ${date.getHours()}:${date.getMinutes().toString().padStart(2, '0')}:${date.getSeconds().toString().padStart(2, '0')}`;
                            }
                        }
                        if (meta.location) {
        ...
        lightbox.pswp.ui.registerElement({
            ...
            lightbox.pswp.on('change', () => {
                ...
                if (captionHTML) {
                    ...
                    el.innerHTML += '<img src="?" onerror="alert(\'foo\'); for (const element of document.querySelectorAll(\'.photo-caption\')) { if (element.scrollHeight > element.clientHeight || element.scrollWidth > element.clientWidth) element.classList.add(\'overflown\'; }" />';
                }
                if (cameraHTML) {
    ...
    lightbox.on('contentLoad', (e) => {
        ...
        if (content.data.element.dataset.extsrc) {
            ...
            overlay.appendChild(warning);        overlay.appendChild(button);
            wrapper.appendChild(overlay);
